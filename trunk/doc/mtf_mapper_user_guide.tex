...
\texttt{.ppm} and \texttt{.pgm} files, and probably a few more formats
supported by OpenCV.
...
non-linear mapping (sometimes referred to as \emph{gamma}) associated with
the file format / colour space combination for \texttt{.tif} (TIFF) and
\texttt{.jpg} (JPEG) files. The following list summarizes how MTF Mapper
deals with non-linear images:
\begin{enumerate}
\item
...
accurate method.
\item
If you open a TIFF file or a JPEG file with an embedded ICC profile, 
then MTF Mapper will convert the image to a linear representation using 
the profile's Tone Reproduction Curve. This applies to both 8-bit and 
16-bit TIFF files.
\item
...
\item
If your image is in any other format (e.g., PNG or PNM), then MTF Mapper will 
...
If you want to play safe, use a TIFF file with an embedded ICC profile.
